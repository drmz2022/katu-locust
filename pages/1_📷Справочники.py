...
st.sidebar.info(
    """
  AP08052747 Подпрограмма 102 «Грантовое финансирование научных исследований». Приоритетное направление «Устойчивое развитие агропромышленного комплекса и безопасность сельскохозяйственной продукции»
    """
)
...
def app():
    ...
    st.title("Методика ДЗЗ")

    st.markdown(
        """
        Комбинации сельскохозяйственных индексов КСЯ
    """
    )
    ...
    with row1_col2:
        ...
        keyword = st.text_input("Выберите экспериментальный полигон:", "")
        if keyword:
        ...
        collection = st.selectbox(
            "Выберите источников спутниковых снимков: ",
            [
    ...
    with row1_col1:
        ...
        with st.expander(
            "Инструкция: Используемые комбинации сельскохозяйственных индексов👉"
        ):
          #  video_empty = st.empty()
            image = 'satelliteimgs/ЗИКР.jpg'
            st.image(image,caption="сельскохозяйственные индексы")

        data = st.file_uploader(
            "Загрузка зоны интересов в формате geojson, kml, zip 😇👇",
            ...
            with st.form("submit_naip_form"):
                ...
                if submitted:
                    ...
                    if sample_roi == "Uploaded GeoJSON" and data is None:
                        ...
                        empty_text.text("Подождите... Идет просчет...")
                        try:
