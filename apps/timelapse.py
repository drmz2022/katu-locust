...
def app():
    ...
    st.markdown(
        """
      Комбинация сельскохозяйственных индексов ДЗЗ
    """
    )
    ...
    with row1_col2:
        ...
        keyword = st.text_input("Поиск экспериментального полигона:", "")
        if keyword:
        ...
        collection = st.selectbox(
            "Выберите источник спутникых снимков: ",
            [
        ...
        if collection == "Any Earth Engine ImageCollection":
            ...
            if asset_id:
                with st.expander("Настройки визуализации", True):
                    try:
                    ...
                    bands = st.multiselect(
                        "Выберите 1 или 3 бэнда (RGB):", img_bands, default_bands
                    )
                    ...
                    if bands:
                        ...
                        vis_params = st.text_area(
                            "Введите параметры визуализации",
                            "{}",
                        )
                    try:
    ...
    with row1_col1:
        ...
        with st.expander(
            "Методика: Используемые комбинации индексов👉"
        ):
